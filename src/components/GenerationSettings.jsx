...
export default function GenerationSettings({
  ...
  const summary = `${style.name} · ${resolution}px · ${aspectRatio} · ${model === "flux" ? "Flux" : model === "turbo" ? "Turbo" : "Sana"} · ${seed === "manual" ? manualSeed || "Manual" : "🎲 Auto"}`;

  return (
    <div className="space-y-4 mt-4">
      <button
        onClick={() => setOpen(!open)}
        className="flex items-center justify-between w-full border-b border-slate-700/50 pb-2 group"
      >
